...
<style>
    ...
    /* Metric Value - Further reduced size */
    [data-testid="stMetricValue"] {
        font-weight: 600; /* Slightly less bold */
        font-size: 1.7em; /* Further reduced size */
        color: #1E1E1E;
        white-space: nowrap;
        overflow: hidden;
        text-overflow: clip;
    ...
    [data-testid="stMetricDelta"] { font-weight: 500; font-size: 0.9em; padding-top: 0.25rem; } /* Slightly smaller delta */
...
# --- Data Fetching Function (World Gold USD/Ounce) ---
@st.cache_data(ttl=CACHE_TTL_SECONDS)
def fetch_world_gold_usd(start_date, end_date):
    """Fetches world gold data in USD/Ounce. Returns (dataframe, raw_dataframe, error_type)"""
    try:
        time.sleep(0.5)
        ...
        processed_data = gold_data_raw.copy()
        ...
        if 'Close' not in processed_data.columns:
             print("Error: 'Close' column not found in yfinance data after processing.")
             return pd.DataFrame(), gold_data_raw, "other"
        gold_data_for_chart = processed_data[['Close']].copy()
        gold_data_for_chart.rename(columns={'Close': 'Giá TG (USD/oz)'}, inplace=True)
        gold_data_for_chart.reset_index(inplace=True)
        index_name = gold_data_for_chart.columns[0]
        gold_data_for_chart.rename(columns={index_name: 'Timestamp'}, inplace=True)
        if 'Timestamp' not in gold_data_for_chart.columns:
             print(f"Error: 'Timestamp' column not created. Index name was {index_name}")
             return pd.DataFrame(), gold_data_raw, "other"
        gold_data_for_chart['Timestamp'] = pd.to_datetime(gold_data_for_chart['Timestamp'])
        return gold_data_for_chart, gold_data_raw, None
...
st.caption(f"Giá TG (USD/oz) & Giá SJC (VND/cây) | Khoảng thời gian: {final_label}")
st.write("")
